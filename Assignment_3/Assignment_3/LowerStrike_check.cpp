...
double LowerStrike_check(string x, double UpperStrike)
{
	...
	while (a<6) {
		...
		rewind(stdin);
		fgets(s, 80, stdin);
		if (2 == sscanf_s(s, "%lf%c", &d, &c) && strtod(s, &step) < UpperStrike && strtod(s, &step) > 0)
		{
			if ('\n' == c) 
			{
				break;
		...
		if ('\n' == s[strlen(s) - 1] && strtod(s, &step) == 0)
		{
			...
			if (a < 5)
			{
				cout << x.c_str() << " Input must a positive number, please try again, " << 5 - a << " attempts left" << endl;
			}
			else
			{
		...
		else if (strtod(s, &step) >= UpperStrike)
		{
			s[strlen(s) - 1] = 0;
			++a;
			if (a<5)
			{
				...
				{
					cout << x.c_str() << " Input can't be higher than or equal to Upper Strike, please try again  " << 5 - a << " attempts left" << endl;
				}
			}
			else
			{
